...
#TODO
# Clean up code, remove tester() function and chnage main.py
# Maybe do the threading thing if REDIS can't do that
# Implement landing algorithm, calculate 
# Circle around when the drone gets too close
# Simulate speed change in drone by changing boat's speed
...
def follow_boat(connection, lat, lon, alt):
    ...
    waypoint1 = (lat, lon, alt)
    #waypoint2 = (lat, lon, alt)

    print(f"Uploading mission with 3 waypoints: , {waypoint1}, ")
    upload_mission(connection, [waypoint1, waypoint1])

    set_mode(connection, "FBWB")
...
def tester():
    ...
    set_mode(drone_connection, "FBWB")
    ...
    drone_connection.mav.command_long_send(
        drone_connection.target_system, drone_connection.target_component,
        mavutil.mavlink.MAV_CMD_DO_SET_SERVO, 0, 3, 1800, 0, 0, 0, 0, 0
    )
    ...
    while True:
        ...
        if i > 9:
            target_lat, target_lon = calc_look_ahead_point(lat, lon, heading, 50)
            follow_boat(drone_connection, target_lat, target_lon, 10)
